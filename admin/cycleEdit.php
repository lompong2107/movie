...
$sql = "SELECT * FROM cycle_detail WHERE cyc_id = " . $_GET["id"];
$query = $conn->query($sql);
$result = mysqli_fetch_assoc($query);
$sqlCyc = "SELECT * FROM cycle WHERE cyc_id = " . $_GET["id"] . " ORDER BY cyc_time";
...
<body>
    ...
    <div class="home-section">
        <nav class="navbar navbar-dark bg-dark home-content w-100 text-white">
            <ul class="navbar-nav flex-row">
                ...
                <li class="nav-item">
                    <span class="h3" id="text-title">แก้ไขรอบฉาย</span>
                </li>
            </ul>
        </nav>
        <div class="content pt-3" id="content">
            <div class="mx-auto border" style="width: 700px;">
                ...
                <form id="cyc_form_edit" method="POST">
                    <div class="row align-items-center mb-3">
                        <div class="col-4 text-end">
                            <label for="mov_id" class="col-form-label">ชื่อภาพยนตร์</label>
                        </div>
                        <div class="col-6">
                            <input type="text" name="cyc_id" id="cyc_id" value="<?= $result["cyc_id"]; ?>" hidden>
                            <select class="form-select" name="mov_id" id="mov_id" required>
                                <option value="">-- โปรดเลือก --</option>
                                <?php
                                while ($row = mysqli_fetch_assoc($queryMov)) {
                                ?>
                                    <option value="<?= $row["mov_id"]; ?>" <?php if ($result["mov_id"] == $row["mov_id"]) echo "selected" ?>><?= $row["mov_name"]; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row align-items-center mb-3">
                        ...
                        <div class="col-6">
                            <input type="date" name="cyc_date" id="cyc_date" class="form-control" value="<?= $result["cyc_date"]; ?>" required>
                        </div>
                    </div>
                    <div class="row align-items-center mb-3">
                        <div class="col-4 text-end">
                    ...
                    <div class="row align-items-center mb-3">
                        ...
                        <div class="col-6 alertTime">
                            <?php while ($row = mysqli_fetch_assoc($queryCyc)) { ?>
                                <div class="alert alert-dark alert-dismissible fade show py-0" role="alert">
                                    <span class="align-middle"><?= $row["cyc_time"]; ?></span>
                                    <button type="button" class="align-middle btn-close btn-sm" onClick="removeItem(this.id)" id="<?= $row["cyc_time"] ?>" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            <?php } ?>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // ! ฟอร์มแก้ไขข้อมูล
            $("#cyc_form_edit").submit(function(e) {
                e.preventDefault();
                $.ajax({
                    type: "POST",
                    url: "cycleController.php",
                    data: $(this).serialize() + "&cycTime=" + data_time + "&submit=cycEdit",
    ...
    <?php
    $sqlCyc = "SELECT * FROM cycle WHERE cyc_id = " . $result["cyc_id"];
    $queryCyc = $conn->query($sqlCyc);
    while ($row2 = mysqli_fetch_assoc($queryCyc)) {
        echo "<script>
        data_time.push(" . json_encode($row2["cyc_time"]) . ");
        </script>";
    }
    ?>
</body>
